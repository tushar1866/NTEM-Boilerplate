...
const forgotPassword = catchAsync(async (req: Request, res: Response) => {
  ...
  res.status(httpStatus.NO_CONTENT).send({ body: req.body.email, resetPasswordToken });
});
...
const sendVerificationEmail = catchAsync(async (req: Request, res: Response) => {
  const verifyEmailToken = await tokenService.generateVerifyEmailToken((req as CustomReq).user);
  // await emailService.sendVerificationEmail((req as CustomReq).user.email, verifyEmailToken);
  res.status(httpStatus.NO_CONTENT).send({ email: (req as CustomReq).user.email, verifyEmailToken });
});
